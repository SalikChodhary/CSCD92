...
int main() {
    ...
    // Calculate eigenvalues using LAPACKE's LAPACKE_dsyev routine
    char jobz = 'N'; // Compute eigenvalues only, not eigenvectors
    char uplo = 'L'; // Use lower triangular part of A
    int lda = n;     // Leading dimension of A
    std::vector<double> eigenvalues(n);
    int info;

    info = LAPACKE_dsyev(LAPACK_ROW_MAJOR, jobz, uplo, n, A.data(), lda, eigenvalues.data());
    ...
    if (info == 0) {
        ...
        if (eigenvaluesFile.is_open()) {
            ...
            // Calculate Gershgorin intervals and check eigenvalues
            for (int i = 0; i < n; ++i) {
